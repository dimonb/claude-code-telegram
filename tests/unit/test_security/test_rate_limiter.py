...
class TestRateLimitBucket:
    ...
    def test_bucket_creation(self):
        ...
        bucket = RateLimitBucket(
            capacity=10, tokens=10, last_update=utc_now(), refill_rate=1.0
        )
    ...
    def test_token_consumption(self):
        ...
        bucket = RateLimitBucket(
            capacity=10, tokens=5, last_update=utc_now(), refill_rate=1.0
        )
    ...
    def test_wait_time_calculation(self):
        ...
        bucket = RateLimitBucket(
            capacity=10, tokens=2, last_update=utc_now(), refill_rate=1.0
        )
    ...
    def test_bucket_status(self):
        ...
        bucket = RateLimitBucket(
            capacity=10, tokens=7, last_update=utc_now(), refill_rate=2.0
        )
...
class TestRateLimiter:
    ...
    async def test_cost_limit_exceeded(self, rate_limiter):
        ...
        rate_limiter.cost_reset_time[user_id] = utc_now()  # Prevent reset
    ...
    async def test_cleanup_inactive_users(self, rate_limiter):
        ...
        # Create old bucket
        bucket = rate_limiter._get_or_create_bucket(user_id)
        bucket.last_update = utc_now() - timedelta(hours=25)
